...
推荐从 GitHub Release 下载 DMG 安装包：

- [下载最新 Release](https://github.com/QingZzao/userscript-manager-safari-extension/releases/latest)

安装步骤：

1. 下载 `UserScriptManagerSafari-*.dmg`。
2. 打开 DMG，把 `UserScriptManagerSafari.app` 拖到 `Applications`。
3. 首次打开如果 macOS 提示“无法验证开发者”，右键点击 App，选择“打开”，或到“系统设置 > 隐私与安全性”中允许打开。
4. 打开 App 后，到 Safari 设置里的“扩展”中启用 `UserScript Manager Safari`。
5. 点击 Safari 工具栏里的扩展按钮，进入管理页面安装用户脚本。

注意：当前 DMG 是免费的未 Developer ID 签名、未公证测试安装包，不需要 Apple Developer Program。普通用户首次打开时可能会看到 macOS 安全提示，这是预期行为。

如果 Safari 里看不到扩展，请确认：

- App 已经放在 `/Applications` 并打开过一次。
- Safari 设置的“扩展”里已勾选 `UserScript Manager Safari`。
- 如果仍看不到，确认 Safari 已开启“开发”菜单，并允许未签名扩展。

重要限制：当前 DMG 是未签名测试安装包。Safari 退出后会重置 `Allow unsigned extensions`，因此再次打开 Safari 后可能需要重新允许未签名扩展。

## 免费账户/未签名版本的临时加载方式

如果你只有免费 Apple Developer 账户，或 Safari 里看不到 DMG 安装的未签名扩展，可以直接加载 Web Extension 源码目录：
...
1. 打开 Safari 设置里的“开发”菜单。
2. 勾选 `Allow unsigned extensions`。
3. 选择 `Add Temporary Extension...`。
4. 选择本项目的 `extension/` 文件夹。
5. 在 Safari 设置的“扩展”中启用它。

临时扩展在 Safari 退出后会被移除，下次需要按同一路径重新加载。

## 从源码构建

如果你想自己构建，或修改代码后本机安装，可以使用项目脚本构建并复制到固定位置：
...
开发调试时，Xcode 的构建产物通常在 `/private/tmp` 或 DerivedData 目录里。这个位置不适合长期给 Safari 识别，重启或系统清理后可能出现“扩展暂时消失，之后又回来”的现象，所以推荐使用上面的稳定安装脚本。
